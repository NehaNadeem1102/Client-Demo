...
const Products = ({ products, gap }: ProductsProps) => {
  ...
  const formatPriceToYen = (price: number) => {
  ...
  return (
    <div>
      <div
        className="relative cursor-pointer shadow-sm shadow-lightDim 
            rounded-md h-32 w-32 sm:h-40 sm:w-40 lg:h-56 lg:w-56 my-10"
      >
        {/* === IMAGE */}
          ...
          <nav className="text-sm font-normal sm:font-medium">
            ...
            <div className="flex gap-3">
              <span className="text-sm text-lightGray line-through">
                {/* Formatting old price */}
                {products.oldPrice && formatPriceToYen(products.oldPrice)}
              </span>
              <b className="text-zinc-900">
                {/* Formatting current price */}
                {formatPriceToYen(products.price)}
              </b>
            </div>
          </nav>
